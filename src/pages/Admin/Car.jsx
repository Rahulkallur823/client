...
const Car = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch('https://food-5mu1.onrender.com/api/v1/product/carousel-images', {
          method: 'GET',
          headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://food-5mu1.onrender.com/api/v1/product/updatec/${selectedImage._id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: AuthorizationToken,
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`https://food-5mu1.onrender.com/api/v1/product/deletec/${selectedImage._id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: AuthorizationToken,
